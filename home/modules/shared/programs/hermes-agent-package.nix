# Vendored from numtide/llm-agents.nix `packages/hermes-agent/package.nix`
# (rev pinned in flake.lock) so we can splice `extraPythonPackages` into
# `hermesDeps` — the upstream numtide package.nix has a closed callPackage arg
# list and does NOT accept extraPythonPackages/.override for it.
#
# Unlike the NousResearch/hermes-agent flake (uv2nix sealed venv with a
# build-time collision guard that aborts on shared transitive deps like
# ddgs->click), this packaging uses `python3.withPackages (_: hermesDeps)`,
# which naturally dedups by canonical name — so adding ddgs/html2text here is
# collision-free even though they transitively re-bring `click`/`pyyaml`/etc.
...
# in hermesDeps, and the doInstallCheck/meta.maintainers optionals).
{
  ...
  nodejs,
  olm,
  # [vendored-change] extra Python modules spliced into hermesDeps so both the
  # `hermes` CLI env (dependencies) and the HERMES_PYTHON gateway env
  # (pythonEnv = python3.withPackages (_: hermesDeps)) can import them.
  extraPythonPackages ? [ ],
}:
