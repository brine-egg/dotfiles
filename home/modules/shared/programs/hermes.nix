...
let
  ...
  # hermes-agent against that exact python3 and pins the same nixpkgs in its
  # lock, so (a) the base package + hermes-frontend npm build hit the numtide
  # binary cache, and (b) ddgs/html2text come from the matching python3Packages
  # — a different nixpkgs' python3 wheels are not cross-interpreter importable.
  # Hermes officially targets Python 3.11–3.13, but numtide's nixpkgs defaults
  # python3 to 3.14 and numtide patches around the 3.14 ThreadPoolExecutor
  # breakage (daemon-pool-python314.patch, now carried by the vendored
  # package.nix). Use numtide's default python3 so the base package + frontend
  # + python deps hit numtide's binary cache. All deps (hermesDeps,
  # extraPythonPackages, mnemosyne) must use the same interpreter — a
  # different nixpkgs' python3 wheels are not cross-interpreter importable.
  numtidePkgs = inputs.llm-agents.inputs.nixpkgs.legacyPackages.${system};
  numtidePython = numtidePkgs.python3;
  numtidePythonPackages = numtidePython.pkgs;

  # Built against the same interpreter hermes-agent runs under so both
  # packages are importable in the CLI and gateway envs.
